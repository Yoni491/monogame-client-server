...
namespace Game1
{
    class HealthManager
    {
        ...
        public HealthManager(int total_health,Vector2 position)
        {
            ...
            _position = position;
            _healthbar = new Texture2D(Program.game.GraphicsDevice, healthbar_width, 1);
            Color[] data = new Color[healthbar_width];
            for (int i = 0; i < data.Length; ++i) data[i] = Color.Green;
            _healthbar.SetData(data);
            _healthbar_background = new Texture2D(Program.game.GraphicsDevice, healthbar_width + 2, 3);
